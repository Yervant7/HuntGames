...
static inline int is_recommended(const DRIVER_REGION_INFO * rinfo) {
    if (strstr(rinfo->name, "[anon:libc_malloc]") || strstr(rinfo->name, "[anon:.bss]") || strstr(rinfo->name, "/data/app/") ||
            is_Ch(rinfo) || is_A(rinfo) || is_S(rinfo) || (strstr(rinfo->name, "/dev/ashmem/") && !strstr(rinfo->name, "dalvik")) ||
            is_rw_0(rinfo)) {
        return 1;
    }
    return 0;
